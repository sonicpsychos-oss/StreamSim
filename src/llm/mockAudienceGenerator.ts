...
import { RealismSignalModel } from "./realismSignals.js";
...
function engagementFromSignals(config: SimulationConfig, context: StreamContext): number {
  const features = realismModel.extract(context, config.persona);
  return Math.min(1.5, 0.6 + features.excitementScore * 0.7 + features.personaBiasScore * 0.2);
}
...
function realisticDonation(config: SimulationConfig, tone: ToneSnapshot, context?: StreamContext): { donationCents?: number; ttsText?: string } {
  ...
  const engagement = engagementFromSignals(config, safeContext);
  const effectiveRate = Math.min(0.82, config.donationFrequency * engagement * (0.45 + features.donationPropensity));
  ...
  const personaScalar = 0.7 + features.personaBiasScore * 0.8;
...
export function generateAudienceBatch(config: SimulationConfig, tone: ToneSnapshot, context?: StreamContext): ChatMessage[] {
  ...
  return Array.from({ length: count }, (_, idx) => {
    ...
    const message: ChatMessage = {
      ...
      text: withBias(text, config.bias === "split" && features.personaBiasScore < 0.45 ? "disagree" : config.bias),
      emotes: Math.random() > 0.45 ? [pick(emotePool)] : [],
      createdAt: new Date().toISOString()
    };

    Object.assign(message, realisticDonation(config, tone, safeContext));
